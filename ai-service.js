...
class AIService {
  ...
  static async testConnection(key, provider = 'gemini') {
    ...
    if (provider === 'gemini') {
      ...
      throw new Error(`Gemini API Error: ${lastError || 'Invalid API Key or unsupported model'}`);
    } else {
      // OpenAI Test
...
Question object structure:
{
  ...
  "answerKey": "Key answer guidance string..."
    ...
    if (provider === 'gemini') {
      const candidateModels = [this.activeGeminiModel, 'gemini-1.5-flash', 'gemini-2.5-flash', 'gemini-1.5-pro'].filter(Boolean);
      ...
      if (!successData) {
        throw new Error(lastErr || "Failed to generate content using Gemini API models.");
      }

      jsonResponseText = successData.candidates?.[0]?.content?.parts?.[0]?.text || "";
    } else {
      // OpenAI API call
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        method: 'POST',
